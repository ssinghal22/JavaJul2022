...
//same as other, maybe better code. Assume each list member is a single digit code.
...
public class AddTwoLinkedLists2 {
    ...
    private static List<Integer> addTwoLists(LinkedList<Integer> list1, LinkedList<Integer> list2){
        ...
        while (size1 >= 0 & size2 >= 0){
            int sum = list1.get(size1) + list2.get(size2) + carryover;
        ...
        if (size1 > 0){
            while (size1 >= 0){
                int sum = list1.get(size1) + carryover;
        ...
        if (size2 > 0) {
            while (size2 >= 0){
                int sum = list2.get(size2) + carryover;
